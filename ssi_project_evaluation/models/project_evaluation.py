...
class ProjectEvaluation(models.Model):
    ...
    batch_id = fields.Many2one(
        comodel_name="project_batch_evaluation",
        string="# Batch",
        ondelete="restrict",
        readonly=True,
    )
    ...
    question_ids = fields.One2many(
        comodel_name="project_evaluation.question",
    ...
    def _01_create_project_evaluation_result(self):
        self.ensure_one()
        if not self.project_id._get_project_evaluation_result(self.type_id):
            data = {
                "project_id": self.project_id.id,
                "type_id": self.type_id.id,
            }
            self.env["project.project.evaluation_result"].create(data)
