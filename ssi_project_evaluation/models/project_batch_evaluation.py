...
class ProjectBatchEvaluation(models.Model):
    _name = "project_batch_evaluation"
    _description = "Project Batch Evaluation"
    _inherit = [
    ...
    project_ids = fields.Many2many(
        comodel_name="project.project",
        relation="rel_project_batch_evaluation_2_project",
        column1="batch_id",
        column2="project_id",
        readonly=True,
        states={"draft": [("readonly", False)]},
        ...
        for record in self:
            ...
            if record.type_id:
                result = record._m2o_configurator_get_filter(
                    object_name="project.project",
                    method_selection=record.type_id.project_selection_method,
                    manual_recordset=record.type_id.project_ids,
                    domain=record.type_id.project_domain,
                    python_code=record.type_id.project_python_code,
                )
            record.allowed_project_ids = result
    ...
    def _open_evaluation(self):
        self.ensure_one()
        action = self.env.ref(
            "ssi_project_evaluation.project_evaluation_action"
        ).read()[0]
    ...
    def _load_project(self):
        self.ensure_one()
        self.write({"project_ids": [(6, 0, self.allowed_project_ids.ids)]})

    @ssi_decorator.post_done_action()
    def _create_evaluations(self):
        self.ensure_one()
        Evaluation = self.env["project_evaluation"]
